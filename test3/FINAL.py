...
# Cargar pesos
model.load_state_dict(torch.load("cvae_lstm_model.pth", map_location="cpu"))  # o "cuda" si usas GPU
model.eval()
...
# === Detectar escala y root note desde progresión original ===
...
# === Generar N versiones y seleccionar la más coherente tonalmente ===
def generar_mejor_progresion(model, x_input, token_to_id, id_to_token, escala_mod12, root_note, N=50, max_len=8):
    model.eval()
    mejores_tokens = None
    mejor_loss = float('inf')  # mejor valor de pérdida
    ...
    with torch.no_grad():
        ...
        for _ in range(N):
            ...
            )[0]  # quitar batch dimension

            # Convertir a lista para manipular
            salida_list = salida.cpu().tolist()

            # Contar tokens válidos (excluyendo <sos>, <eos>, <pad>)
            salida_chords = [
                id_to_token[token] 
    ...
    # Si no se encontró ninguno con la longitud exacta, podemos elegir la mejor sin filtrar o lanzar advertencia
    if mejores_tokens is None:
        print("[⚠️] No se encontró progresión con longitud exacta. Seleccionando mejor entre todas.")
        # Volver a evaluar sin filtro de longitud:
...
def build_midi_from_chords(generated_chords, reference_midi_path, output_path):
    ...
    # Encontrar nota mínima del original para usar como base
    ...
    # Obtener tiempos de inicio y duración promedio de acordes en original
    # Si tienes varias notas, puedes agrupar por tiempo para obtener acordes originales
    # Aquí tomamos duración promedio simple para todos los notes
    ...
    for chord_str in generated_chords:
        ...
        for interval in notes:
            pitch = base_pitch + interval  # Mantener registro relativo al original
            if pitch < 0:  # evitar pitch negativos
                pitch = 0
            elif pitch > 127:
                pitch = 127
    ...
    print(f"[✅] MIDI enriquecido guardado en: {output_path}")

# === Procesar todos los archivos en MIDI_TEST ===
input_dir = "star_test"
output_dir = "star_resultado"
os.makedirs(output_dir, exist_ok=True)

archivos = [f for f in os.listdir(input_dir) if f.lower().endswith(".mid")]
...
for nombre_archivo in archivos:
    ...
    try:
        ...
        new_base_name = "GENERATED_" + re.sub(r'\D+', '', base_name)
        output_path = os.path.join(output_dir, f"{new_base_name}.mid")

        # Preprocesamiento
        x_input = midi_to_chords_train(input_path)
        x_input = progression_to_token_seq(x_input, token_to_id_train)
        x_input = torch.tensor(x_input, dtype=torch.long).unsqueeze(0).to(device)

        if x_input.size(1) < 2:
            print(f"[⚠️] Progresión demasiado corta en {nombre_archivo}, saltando...")
            continue

        entrada_tokens = x_input[0].cpu().tolist()
        notas_midi = [
            60 + int(p)
            for t in entrada_tokens
            if id_to_token_train[t] not in ['<sos>', '<eos>', '<pad>']
            for p in id_to_token_train[t].split('_') if p.isdigit()
        ]

        escala_mod12 = detectar_escala(notas_midi)
        root_note = min(notas_midi) % 12

        mejores_chords = generar_mejor_progresion(
            model, x_input,
            ...
            root_note=root_note,
            N=20,
            max_len=x_input.size(1)
        )

        # Mostrar progresión
        entrada_chords = [
            id_to_token_train[tok]
            for tok in entrada_tokens
            if tok not in [token_to_id_train['<pad>'], token_to_id_train['<eos>'], token_to_id_train['<sos>']]
        ]
        salida_chords = [ch for ch in mejores_chords if ch not in ['<pad>', '<eos>', '<sos>']]

        print(f"🔹 Entrada   : {' | '.join(entrada_chords)}")
        print(f"🔸 Generado  : {' | '.join(salida_chords)}")

        # Guardar MIDI
        build_midi_from_chords(mejores_chords, input_path, output_path)
