...
def obtener_bpm(midi_path):
    pm = pretty_midi.PrettyMIDI(midi_path)
    tempos = pm.get_tempo_changes()[1]
    return tempos[0] if len(tempos) > 0 else 120  # default 120 si no hay info

bpm = obtener_bpm(midi_path)


def build_midi_from_chords(generated_chords, output_path, root_note=None, bpm=120, base_pitch=48, duracion_promedio=0.5):
    """
    Construye un MIDI desde acordes generados, respetando el BPM y la duración promedio
    del MIDI original.
    """
    ...
    for chord_str in generated_chords:
        ...
        if notes:
            ...
            piano.notes.append(pretty_midi.Note(
                velocity=random_vel(DEFAULT_MEAN_VEL + 10, DEFAULT_STD_VEL),
                pitch=target_pitch,
                start=time,
                end=time + duracion_promedio
            ))
...
def ajustar_tempo(midi_path, tempo_objetivo, output_path):
    pm = pretty_midi.PrettyMIDI(midi_path)
    tempo_actual = obtener_bpm(midi_path)
    factor = tempo_actual / tempo_objetivo
...
def midi_to_wav_fluidsynth(midi_path, wav_path=None, sf2_path=SF2_PATH, verbose=True):
    ...
    bpm = obtener_bpm(midi_path)  # aquí obtienes tempo real
...
input_midi_dir = "/Users/pepebeats/Desktop/Semi-Supervised-CVAE-LSTM/test4EVO/originales"
output_best_variantes_dir = "/Users/pepebeats/Desktop/Semi-Supervised-CVAE-LSTM/test4EVO/resultados"
output_best_wav_dir = "/Users/pepebeats/Desktop/Semi-Supervised-CVAE-LSTM/test4EVO/resultados_wav"
output_original_wav_dir = "/Users/pepebeats/Desktop/Semi-Supervised-CVAE-LSTM/test4EVO/originales_wav"

os.makedirs(output_best_variantes_dir, exist_ok=True)
os.makedirs(output_best_wav_dir, exist_ok=True)
os.makedirs(output_original_wav_dir, exist_ok=True)
...
for archivo in archivos_midi:
    try:
        ...
        # WAV original
        wav_original_path = os.path.join(output_original_wav_dir, f"{os.path.splitext(archivo)[0]}_ORIGINAL.wav")
        midi_to_wav_fluidsynth(midi_path, wav_original_path, sf2_path=SF2_PATH)
        print(f"[ℹ️] WAV original generado: {wav_original_path}")

        # Extraer acordes
        chords_prog = midi_to_chords_train(midi_path)
        if not chords_prog:
            print(f"[⚠️] No se detectaron acordes en {archivo}")
            continue
        ...
        if not mejores_variantes:
            print(f"[⚠️] No se generaron variantes para {archivo}")
            continue

        variante, loss = mejores_variantes[0]
        simbolos = [token_to_chord_symbol(ch) for ch in variante]

        duracion_promedio = calcular_duracion_promedio(midi_path)
        bpm_original = obtener_bpm(midi_path)

        midi_output_path = os.path.join(output_best_variantes_dir, f"{os.path.splitext(archivo)[0]}_MEJOR.mid")
        build_midi_from_chords(variante, midi_output_path,
                            root_note=root_note,
                            base_pitch=60,
                            bpm=bpm_original,
        ...
        wav_output_path = os.path.join(output_best_wav_dir, f"{os.path.splitext(archivo)[0]}_MEJOR.wav")
        midi_to_wav_fluidsynth(midi_output_path, wav_output_path, sf2_path=SF2_PATH)

        print(f"[✅] Mejor variante guardada: {archivo} → {midi_output_path}")
        print(f"Progresión: {' | '.join(simbolos)} | Loss: {loss:.4f}")
